...
class ArcFlags:
    ...
    def print_progress(self, current_index: int, total_entries: int):
        """
        Prints the current progress of arc flags preprocessing as a percentage.
        Input:
            current_index (int) : The index of the current node being processed of all nodes
            total_entries (int) : The total number of nodes to be processed
        Return:
            None.
        """
    ...
    def get_edge_nodes(self) -> List[Node]:
        ...
        Input:
            None.
        Output:
            edge_nodes (List[Node]) : List of edge nodes in graph
        """
        edge_nodes = []
    ...
    def preprocess_graph(self):
        ...
        for index, edge_node in enumerate(edge_nodes):
            ...
            for node in nodes:
                # Arc flag index is the nth region of all rectangular partitions
                arc_flag_index = node.region[0] + (
                    node.region[1] * self.graph.num_partitions_axis
                )
                node_neighbors = self.graph.graph[node]

                # Every edge from a node by definition has the arc flag corresponding to region of node set
                ...
                try:
                    ...
                    # set the arc-flags along the shortest path corresponding to the edge node's region
                    for i in range(len(shortest_path) - 1):
                        ...
                        # Get the edge from a source node i to the next node, i + 1, and set edge's arc flag
    ...
    def get_shortest_path(self, end_node: Node, predecessors: dict[Node, Node]) -> list[Node]:
        """
        Returns the shortests path from end node to the start node.
        Input:
            end_node (Node) : The end node of the shortest path
            predecssots (dict[Node, Node]) : Dictiionary of quickest route from Node to Node
        Return:
            path list[Node] : List of shortest path from start node to end node.
        """
        path = []
        current = end_node
    ...
    def arc_flags_dijkstra(self, source: Node, target: Node) -> list[Node]:
        """
        Calculates the shortest path from a source node to other nodes until a shortest path is found
            from source to target. This makes use of a preprocessed arc flags graph that allows for the
            shortest paths to be followed to the region of target node.
        Input:
            source (Node) : Start node of shortest path.
            target (Node) : Target node of shortest path.
        Return:
            (list[Node]) : The shortest path of nodes from a source to target node.
        """
