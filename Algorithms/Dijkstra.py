...
class DijkstraNode:
    def __init__(self, graph: dict[Node, dict[Node, Edge]]):
        ...
        Input:
            ...
            neighbors and Edges to perform search on.
        Return:
            None
        """
        self.graph = graph
    ...
    def compute_shortest_paths(self, start_node: Node):
        """
        Calculates the shortest path to all possible nodes from a start node and store in predecessor.
        Input:
            start_node (Node) : Integer representing start node.
        Return:
            None
        """
    ...
    def get_shortest_path(self, end_node: Node) -> list[Node]:
        """
        Returns the shortests path from end node to the start node.
        Input:
            end_node (Node) : Integer representing end node.
        Return:
            path [(Node)] : List of shortest path from start node to end node.
        """
        path = []
        current = end_node
        while current is not None:
    ...
    def find_shortest_path(self, start_node: Node, end_node: Node) -> list[Node]:
        """
        Function that uses helper functions compute_shortest_paths and get_shortest_path to
        find the shortest path between start node and end node.
        Input:
            start_node (Node) : Integer representing the starting node.
            end_node (Node) : Integer representing the starting node.
        Return:
             List of shortest path from start node to end node.
        """
        self.compute_shortest_paths(start_node)
